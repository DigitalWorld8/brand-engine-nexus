...
const Banner: React.FC<BannerProps> = ({ onBannerClick, visible }) => {
  ...
  useEffect(() => {
    // Hide banner when user has scrolled past threshold or is returning visitor
    if ((hasScrolled && visible && initialScrollBuffer > 50) || isReturningVisitor) {
      onBannerClick();
    }
  }, [hasScrolled, visible, onBannerClick, initialScrollBuffer, isReturningVisitor]);

  // Don't show banner for returning visitors
  if (!visible || isReturningVisitor) return null;
        ...
        >
          <div className="max-w-4xl px-6 text-center">
            <motion.div
              ...
              transition={{ delay: 0.02, duration: 0.15, ease: [0.22, 1, 0.36, 1] }}
            >
            ...
            <motion.p 
              ...
              animate={{ y: 0, opacity: 1 }}
              transition={{ delay: 0.05, duration: 0.15, ease: [0.22, 1, 0.36, 1] }}
            >
            ...
            <motion.div
              ...
              initial={{ y: -10, opacity: 0 }}
              animate={{ y: 0, opacity: 1 }}
              transition={{ delay: 0.07, duration: 0.15, ease: [0.22, 1, 0.36, 1] }}
            >
