            ...
            <TabsContent value={activeTab} className="mt-0">
              <div className="bg-white rounded-lg shadow-md divide-y">
                {filteredFAQs.map((faq, index) => (
                  <Collapsible 
                    key={`${activeTab}-${index}`}
                    open={openItems.includes(`${activeTab}-${index}`)}
                    onOpenChange={() => toggleItem(`${activeTab}-${index}`)}
                    className="border-b last:border-b-0"
                  >
                    <CollapsibleTrigger className="w-full flex justify-between items-center px-6 py-5 hover:bg-brand-light-gray/50 text-left transition-colors">
                      <span className="font-medium text-lg">{faq.question}</span>
                      <ChevronDown className={`h-5 w-5 text-gray-500 transition-transform duration-200 ${openItems.includes(`${activeTab}-${index}`) ? 'rotate-180' : ''}`} />
                    </CollapsibleTrigger>
                    <CollapsibleContent className="px-6 pb-6 pt-2 text-gray-600 animate-accordion-down">
                      {faq.answer}
                    </CollapsibleContent>
                  </Collapsible>
                ))}
