...
const Navbar = () => {
  ...
  return <header className={cn("fixed top-0 left-0 right-0 w-full z-[9999]", "transition-all duration-500 ease-in-out", "will-change-transform", isScrolled ? "bg-white shadow-md" : "bg-transparent")}>
      {/* When not scrolled, show logo absolutely positioned in the primary brand color area */}
      {!isScrolled ? <div className="absolute top-12 left-1/2 transform -translate-x-1/2 z-20">
          <div className="bg-brand-primary py-6 rounded-full shadow-lg logo-glow logo-float px-[30px] my-[26px]">
