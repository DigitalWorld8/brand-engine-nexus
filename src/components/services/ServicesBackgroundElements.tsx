...
const ServicesBackgroundElements = ({ activeService }: ServicesBackgroundElementsProps) => {
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      {/* Gradient blobs */}
      <div className={`absolute top-40 -right-40 w-96 h-96 rounded-full bg-gradient-to-br from-brand-accent-blue/10 to-brand-primary/5 blur-3xl opacity-70 transition-all duration-700 ${activeService ? 'scale-110' : ''}`}></div>
      <div className={`absolute -bottom-20 -left-20 w-96 h-96 rounded-full bg-gradient-to-tr from-brand-accent-violet/10 to-brand-primary/5 blur-3xl opacity-70 transition-all duration-700 ${activeService ? 'scale-110' : ''}`}></div>
      
      {/* Center glow */}
      <div className={`absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-full h-full max-w-6xl max-h-96 opacity-5 transition-opacity duration-700 ${activeService ? 'opacity-10' : ''}`}>
