...
const Hero = () => {
  ...
  // Determine if CTAs should be enabled based on scroll state and first scroll
  const ctasEnabled = isScrolled || hasCompletedFirstScroll;
  
  return <section className={`hero-section relative pt-40 pb-20 md:pt-56 md:pb-32 overflow-hidden mx-0 my-[45px] px-0 py-[320px] ${!isScrolled ? 'hero-tear-shape' : ''}`}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        <div className="grid md:grid-cols-2 gap-12 items-center">
          <div className="space-y-8 md:pr-12">
            <div className="animate-fade-in-up">
              ...
              <div className={ctasEnabled ? '' : 'blur-on-load'}>
                ...
                animationDelay: '0.4s'
                  ...
                  {!ctasEnabled && <div className="absolute inset-0 bg-white/30 backdrop-blur-md z-10 pointer-events-none transition-all duration-500"></div>}
                  
                  <Button size="lg" className={`btn-brand-primary hover:scale-105 transition-transform relative overflow-hidden group ${!ctasEnabled ? 'cta-disabled' : 'cta-enabled'}`} onMouseEnter={() => ctasEnabled && setHovered('primary')} onMouseLeave={() => setHovered(null)} disabled={!ctasEnabled}>
                  ...
                  <Button size="lg" variant="outline" className={`border-brand-accent-blue text-brand-accent-blue hover:bg-brand-accent-blue/10 hover:scale-105 transition-all ${!ctasEnabled ? 'cta-disabled' : 'cta-enabled'}`} onMouseEnter={() => ctasEnabled && setHovered('secondary')} onMouseLeave={() => setHovered(null)} disabled={!ctasEnabled}>
          ...
          <div className={`relative ${ctasEnabled ? '' : 'blur-on-load'}`}>
        ...
        <div className={`mt-16 md:mt-24 flex flex-wrap justify-center md:justify-between items-center gap-8 text-center md:text-left ${ctasEnabled ? '' : 'blur-on-load'}`}>
